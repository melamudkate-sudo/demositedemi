...
for(const [width,height] of [[1920,1080],[1440,900],[1280,640],[390,844],[320,568]]){
 ...
 const sizes=await p.locator('.chapter-title').evaluateAll(es=>es.map(e=>getComputedStyle(e).fontSize));assert.equal(sizes.length,12);assert.equal(new Set(sizes).size,1,`${width} chapter headings`);
...
await p.locator('.production-video').click();await p.locator('.film-close').click();assert.equal(await p.locator('body').evaluate(e=>e.classList.contains('film-open')),false);
...
await p.emulateMedia({reducedMotion:'no-preference'});await p.locator('#commercial').evaluate(e=>e.scrollIntoView({behavior:'instant'}));await p.waitForTimeout(2500);const distributor=await p.locator('.commercial-distributor').evaluate(e=>({background:getComputedStyle(e).backgroundColor,shadow:getComputedStyle(e).boxShadow}));assert.deepEqual(distributor,{background:'rgba(0, 0, 0, 0)',shadow:'none'});
console.log('PASS heading parity, compact chapters, modal keyboard/backdrop/focus, stable distributor');await b.close();})().catch(e=>{console.error(e);process.exit(1)});
